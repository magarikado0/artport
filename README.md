...
プロジェクトを実行する前に、`.env` ファイルを作成し、Firebase Consoleから必要な環境変数を設定してください。サンプルとして `.env.example` ファイルが用意されています。
...
プロジェクトを実行する前に、`.env` ファイルを作成し、必要な環境変数を設定してください。サンプルとして `.env.example` ファイルが用意されています。

```bash
cp .env.example .env
```

その後、`.env` ファイルに適切な値を入力してください。
